...
export async function PUT(
  ...
  if (body.hero_copy !== undefined)
    updates.hero_copy = body.hero_copy?.trim() || null;
  if (body.hero_image_alt !== undefined)
    updates.hero_image_alt = body.hero_image_alt?.trim() || null;
  ...
  // temporarily not using .single() so we can see the raw array
  if (error) {
    const msg =
      error.code === "23505"
